...
class TestFilesPush(TestCase):
    """
    Tests the file push operation and internally mountdir and swift fuctionality
    """
    ...
    def setUp(self):
        ...
        self.d_msg = {'meta': {'remote': {'key': '121215'}, 'local': {'path': '/tmp/datadir'}, 'transport': {'mechanism': 'compress', 'compress': {'cleanup': True, 'unpack': True, 'encoding': 'base64', 'archive': 'zip'}, 'checkRemote': False}}, 'action': 'pushPath'}
    ...
    @patch('swift_store.SwiftStore.doZipping')
    @patch('swift_store.SwiftStore._initiateSwiftConnection')
    @patch('swift_store.SwiftStore._putObject')
    @patch('swift_store.SwiftStore._deleteEmptyDirectory')
    def test_swift_store_store_data(self, mock_deleteEmptyDirectory, mock_putObject, mock_SwiftConnectionInitialization, mock_zipUp):
        ...
        assert mock_deleteEmptyDirectory.call_count == 1        
...
class TestFilesPull(TestCase):
    """
    Tests the file pull operation and internally mountdir and swift fuctionality
    """
    ...
    def test_doGet(self, mock_urllib, mock_getCopy, mock_getCompress, mock_getRemoteStatus):
        ...
        mock_urllib.return_value = {'action': 'pullPath', 'meta': "{'local': {'path': '/tmp/localstore'}, 'transport': {'compress': {'unpack': True, 'encoding': 'base64', 'cleanup': True, 'archive': 'zip'}, 'mechanism': 'compress'}, 'remote': {'key': '121215'}}"}
    ...
    def test_doGetwithCompression(self, mock_retClient, mock_osPathIsDir, mock_get_data, mock_remoteLocation_resolve):
        ...
        d_ret = self.storeObject.do_GET_withCompression(
                        {'action': 'pullPath',
                         'meta'  : {"remote":{ "key":"121215"},"local":{"path":"/tmp/localstore"},"transport":{"mechanism":"compress","compress":{"encoding":"base64","archive":"zip","unpack":True,"cleanup":True}}}})
